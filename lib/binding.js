...
const {
	...
	promiseToCallback,
	withHeapBuffer,
	withStringOrBufferAsHeapBuffer,
} = require('./util');
...
module.exports = function(constants, fsPointer) {
	...
	async function mkdir(path, mode) {
		return await withStringOrBufferAsHeapBuffer(path, async (heapBufferPointer) => {
	...
	async function readdir(path, encoding) {
		...
		await withStringOrBufferAsHeapBuffer(path, async (heapBufferPointer) => {
